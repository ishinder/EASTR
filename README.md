...
- [Emending Alignments of Spliced Transcript Reads (EASTR)](#emending-alignments-of-spliced-transcript-reads-eastr)
  - [Prerequisites](#prerequisites)
  - [Getting Started](#getting-started)
  - [Usage](#usage)
    - [Required Arguments](#required-arguments)
    - [Optional Arguments](#optional-arguments)
    - [Minimap2 Parameters](#minimap2-parameters)
    - [Output Options](#output-options)
    - [Other arguments](#other-arguments)
    - [Running EASTR on a bamlist](#running-eastr-on-a-bamlist)
    - [Running EASTR on a GTF](#running-eastr-on-a-gtf)
  - [Analyzing an example dataset](#analyzing-an-example-dataset)
  - [Citation](#citation)

EASTR is a tool for detecting and eliminating spuriously spliced alignments
in RNA-seq datasets. It improves the accuracy of transcriptome assembly by
identifying and removing misaligned spliced alignments. The tool can process
GTF, BED, and BAM files. EASTR can be applied to any RNA-seq dataset
...
1. Clone repository and submodules
    ...
    ```bash
    # Change EASTR_DIR to any other location of your choosing.
    EASTR_DIR=$HOME/eastr
    git clone --recurse-submodules https://github.com/ishinder/EASTR.git $EASTR_DIR
    cd EASTR
    ```
...
3. Add the junction_extractor and vacuum executables in the
[`utils`](./utils/) folder to your PATH environment variable.

    ```bash
    export PATH=$PATH:$EASTR_DIR/utils
    ```

    To store this path permanently update your bashrc/bash_profile/zshrc or
    other shell rc file.

    ```bash
    # ~/.bashrc
    export EASTR_DIR=$HOME/eastr
    export PATH=$PATH:$EASTR_DIR/utils
    ```

1. Install EASTR globally or in a virtual environment
...
- `-o` : Length of the overhang on either side of the splice junction.
Default: 50
...
shift must meet or exceed to be considered as representing duplicated exons. It
is used to differentiate between exon duplications and spurious splice
...
pipeline to an *Arabidopsis* dataset featured in our study.
The `sra_list_arabidopsis.txt` file, located in the [`tests`](./tests/)
directory, lists the accession IDs of the samples analyzed.

The EASTR pipeline takes BAM files as input. The
[`run_all.sh`](./tests/run_all.sh) script acquires FASTQ files, the FASTA
reference and annotation, and then aligns the FASTQ files using HISAT2 to
generate BAM files. These BAM files are subsequently used as input to EASTR.
Additionally, EASTR can accept a GTF annotation file and output a BED file
containing questionable junctions (executed in the last command of the
[`run_eastr.sh`](./tests/run_eastr.sh) script).

To execute the entire EASTR pipeline, which filters BAM files and identifies
reference annotation errors, use the [`run_all.sh`](./tests/run_all.sh) script
found in the [`tests`](./tests/) directory. This script ensures all necessary
steps and subscripts are carried out in the correct order. To analyze the
example dataset, follow these steps:

1. Navigate to the [`tests`](./tests/) directory within the EASTR package:
2. Make sure all scripts are executable (`chmod +x *sh`):
3. Run the [`run_all.sh`](./tests/run_all.sh)  script.
...
their respective directories within the [`tests`](./tests/) folder.
...
**Note 1**: Downloading FASTQ files using the
[`get_fastq.sh`](./tests/get_fastq.sh) script requires
[SRA_toolkit](https://github.com/ncbi/sra-tools)

**Note 2**: Converting the GFF reference annotation to GTF in the
[`get_ref.sh`](./tests/get_ref.sh) script requires
...
systematic alignment errors in multi-exon genes. Nat Commun.
2023 Nov 9;14(1):7223. doi: [10.1038/s41467-023-43017-4](https://doi.org/10.1038/s41467-023-43017-4).
PMID: [37940654](https://pubmed.ncbi.nlm.nih.gov/37940654/);
PMCID: [PMC10632439](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC10632439/).
