...
def out_junctions_filelist(
    bam_list: list, gtf_path, bed_list, out_junctions, suffix=""
  ...
  if gtf_path:
    if utils.check_directory_or_file(out_junctions) == "dir":
      out_junctions = (
          out_junctions
          + "/"
          + os.path.splitext(os.path.basename(gtf_path)[0])
          + suffix
          + ".bed"
      )
  ...
  if bed_list:
    ...
    if utils.check_directory_or_file(out_junctions) == "file":
      print(
          "ERROR: the path provided for the output bed files is a file path, not a directory"
      )
      sys.exit(1)
    ...
    for bed in bed_list:
      result.append(
          f"{out_junctions}/{os.path.splitext(os.path.basename(bed))[0]}{suffix}.bed"
      )
    return result
...
def out_filtered_bam_filelist(
    bam_list: list, out_filtered_bam, suffix="_EASTR_filtered"
) -> Union[List[str], None]:
  result = None
  ...
  if len(bam_list) == 1:
    if utils.check_directory_or_file(out_filtered_bam) == "dir":
      out_filtered_bam = (
          out_filtered_bam
          + "/"
          + os.path.splitext(os.path.basename(bam_list[0]))[0]
          + suffix
          + ".bam"
      )
    ...
    if utils.check_directory_or_file(out_filtered_bam) == "file":
      print(
          "ERROR: the path provided for the output file is a file, not a directory"
      )
      sys.exit(1)
...
def spurious_dict_all_to_bed(
    spurious_dict, scoring, fileout, gtf_path, bed_list, bam_list
):
...
def create_sample_to_bed_dict(sample_names, out_removed_junctions_filelist):
  ...
  for sample in sample_names:
    ...
    if shortest_match is not None:  # Only update if a match was found
      file_path = out_removed_junctions_filelist[
          out_removed_junctions_filelist.index(shortest_match)
      ]
...
def spurious_dict_bed_by_sample_to_bed(
    spurious_dict, bed_list, out_removed_junctions_filelist, scoring
):
  sample_names = [
      os.path.splitext(os.path.basename(bed_path))[0] for bed_path in bed_list
  ]
  sample_to_bed = create_sample_to_bed_dict(
      sample_names, out_removed_junctions_filelist
  )
  ...
  for i, key in enumerate(sorted_keys):
    ...
    score2 = alignment_utils.calc_alignment_score(
        spurious_dict[key]["hit"], scoring
    )
    name = "JUNC{:0{}d}".format(i + 1, num_digits)

    for _, name2, score in samples:
      sample_to_bed[name2].write(
          f"{chrom}\t{start}\t{end}\t{name}\t{score}\t{strand}\t{name2}\t{score2}\n".encode()
      )
...
def spurious_dict_bam_by_sample_to_bed(
    spurious_dict, bam_list, out_removed_junctions_filelist, scoring
):

  # get sample name from bam_list
  sample_names = [
      os.path.splitext(os.path.basename(bam_path))[0] for bam_path in bam_list
  ]
  ...
  if out_removed_junctions_filelist is None:
    for sample in sample_names:
      sample_to_bed[sample] = tempfile.NamedTemporaryFile(
          delete=False, dir="tmp", suffix=".bed"
      )
  ...
  for i, key in enumerate(sorted_keys):
    ...
    named_keys[(name,) + key]["score2"] = alignment_utils.calc_alignment_score(
        spurious_dict[key]["hit"], scoring
    )
  ...
  for (name, chrom, start, end, strand), value in named_keys.items():
    ...
    for sample, score in samples:
      sample_to_bed[sample].write(
          f"{chrom}\t{start}\t{end}\t{name}\t{score}\t{strand}\t{score2}\n".encode()
      )
...
def filter_bam_with_vacuum(
    bam_path,
    spurious_junctions_bed,
    out_bam_path,
    verbose,
  ...
  vacuum_cmd = (
      f"{vacuum_cmd} -o {out_bam_path} {bam_path} {spurious_junctions_bed}"
  )
  vacuum_cmd = shlex.split(vacuum_cmd)

  # use subprocess to run vacuum_cmd and return stdout and stderr
  process = subprocess.Popen(
      (vacuum_cmd), stdout=subprocess.PIPE, stderr=subprocess.PIPE
  )
...
def filter_multi_bam_with_vacuum(
    bam_list, sample_to_bed, out_bam_list, p, verbose, removed_alignments_bam
):
  ...
  with pool:
    outs = pool.starmap(
        filter_bam_with_vacuum,
        zip(
            bam_list,
            [sample_to_bed[sample] for sample in sample_names],
            out_bam_list,
            verbose,
            removed_alignments_bam,
        ),
