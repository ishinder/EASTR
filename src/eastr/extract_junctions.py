...
def get_junctions_from_bed(bed_path: str) -> dict:
  ...
  with open(bed_path, "r") as f:
    for line in f:
      ...
      if start > end:
        raise Exception(
            "Start of region cannot be greater than end of region for:\n", line
        )
...
def junction_extractor_multi_bam(
    bam_list: list, out_original_junctions: list, p: int
) -> dict:
  with multiprocessing.Pool(p) as pool:
    results = pool.starmap(
        junction_extractor, zip(bam_list, out_original_junctions)
    )
...
def extract_splice_sites_gtf(gtf_path: str) -> dict:
  ...
  for line in gtf:
    ...
    if start > end:
      raise Exception(
          "Start of region can not be greater than end of region for:\n", line
      )
  ...
  for tran, (chrom, strand, gene_id, exons) in trans.items():
    for i in range(1, len(exons)):
      if (
          "transcripts"
          not in junctions[(chrom, exons[i - 1][1], exons[i][0] - 1, strand)]
      ):
        junctions[(chrom, exons[i - 1][1], exons[i][0] - 1, strand)][
            "transcripts"
        ] = [gene_id, [tran]]
      else:
        junctions[(chrom, exons[i - 1][1], exons[i][0] - 1, strand)][
