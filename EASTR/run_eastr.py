...
def parse_args(arglist):
  ...
  group_reqd.add_argument(
      "--gtf", help="Input GTF file containing transcript annotations"
  )
  group_reqd.add_argument(
      "--bed", help="Input BED file with intron coordinates"
  )
  group_reqd.add_argument(
      "--bam",
      help="Input BAM file or a TXT file containing a list of BAM files with read alignments",
  )
  parser.add_argument(
      "-r",
      "--reference",
  ...
  group_mm2.add_argument(
      "-A", help="Matching score. Default = 3", default=3, type=int
  )

  group_mm2.add_argument(
      "-B", help="Mismatching penalty. Default = 4", default=4, type=int
  )
  ...
  group_mm2.add_argument(
      "-k", help="K-mer length for alignment. Default=3", default=3, type=int
  )
  ...
  group_mm2.add_argument(
      "-w", help="Minimizer window size. Default=2", default=2, type=int
  )
  ...
  # other args
  parser.add_argument(
      "-p", help="Number of parallel processes, default=1", default=1, type=int
  )
...
def main(arglist=None):
      ...
      with open(bam_list) as file:
        ...
        for bam in bam_list:
          if not os.path.isfile(bam):
            raise ValueError(
                "input must be a bam file or a file containing a list of bam files"
            )
      ...
      with open(bed_list) as file:
        ...
        for bed in bed_list:
          if not os.path.isfile(bed):
            raise ValueError(
                "input must be a bed file or a file containing a list of bed files"
            )

  original_junctions_filelist = output_utils.out_junctions_filelist(
      bam_list,
      gtf_path,
      bed_list,
      out_original_junctions,
  ...
  removed_junctions_filelist = output_utils.out_junctions_filelist(
      bam_list,
      gtf_path,
      bed_list,
      out_removed_junctions,
  ...
  filtered_bam_filelist = output_utils.out_filtered_bam_filelist(
      bam_list, out_filtered_bam, suffix=suffix
  )

  spurious_dict = get_spurious_introns.get_spurious_junctions(
      scoring,
      k,
      w,
      m,
  ...
  if is_bam:
    if filtered_bam_filelist:
      ...
      output_utils.filter_multi_bam_with_vacuum(
          bam_list,
          sample_to_bed,
      ...
      output_utils.spurious_dict_all_to_bed(
          spurious_dict, scoring, None, gtf_path, bed_list, bam_list
      )
  ...
  if gtf_path:
    ...
    if removed_junctions_filelist:
      output_utils.spurious_dict_bed_by_sample_to_bed(
          spurious_dict, bed_list, removed_junctions_filelist, scoring
      )
    else:
      output_utils.spurious_dict_all_to_bed(
          spurious_dict, scoring, None, gtf_path, bed_list, bam_list
