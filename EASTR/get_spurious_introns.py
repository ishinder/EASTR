...
def bowtie2_align_self_introns_to_ref(
    introns_to_align, seqs, bt2_index, overhang, p=1, len_=15, bt2_k=10
):
  bt2_k = bt2_k + 1
...
def is_spurious_alignment(
    key, value, seqs, overhang, min_duplicate_exon_length, bt2_k=10, anchor=7
):
...
def get_spurious_introns(
  ...
  bw2_alignments = bowtie2_align_self_introns_to_ref(
      introns_to_align, seqs, bt2_index, overhang, p=p, len_=15, bt2_k=bt2_k
  )

  for k, v in introns_to_align.items():
    if is_spurious_alignment(
        k, v, seqs, overhang, min_duplicate_exon_length, anchor=anchor
    ):
      spurious[k] = v
...
def get_spurious_junctions(
  ...
  if bam_list:
    ...
    if out_original_junctions is None:
      remove_tmp = True
      out_original_junctions = [
          tempfile.NamedTemporaryFile(delete=False, dir="tmp", suffix=".bed")
    ...
    introns = extract_junctions.junction_extractor_multi_bam(
        bam_list, out_original_junctions, p
    )

    if verbose:
      end_extr = time.time()
      print(
          "extracting junctions took {} seconds".format(end_extr - start_extr)
      )
  ...
  seqs = alignment_utils.get_flanking_subsequences(
      introns, chrom_sizes, overhang, ref_fa
  )
  self_introns = get_self_aligned_introns(
      introns, seqs, overhang, k, w, m, scoring
  )
  spurious_dict = get_spurious_introns(
      self_introns,
      seqs,
      bt2_index,
      overhang,
  ...
  spurious_dict = dict(
      sorted(
          spurious_dict.items(),
          key=lambda x: (x[0][1], x[0][1], x[0][2], x[0][3]),
      )
  )
  ...
  if verbose:
    end_spur = time.time()
    print(
        "Getting spurious junctions took {} seconds".format(
            end_spur - start_spur
        )
    )
